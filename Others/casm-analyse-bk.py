...
if __name__ == '__main__':
    ...
    if args.type=="vasp" or args.type=="castep":
        #logList=[x[0:-1] for x in popen4('grep -l "Geometry optimization completed successfully." `find ./ -type f -name "*.castep"`','r')[1].readlines()]
        if args.type=="castep":#Get the list of castep files in subfolders for CASTEP.
            #if os.path.exists()
            fileList=[x[0:-1] for x in popen4('find ./ -type f -name "*.castep"','r')[1].readlines()]
        
        elif args.type=="vasp": #Get the list of xml files in subfolders for VASP.
            fileList=[x[0:-1] for x in popen4('find ./ -type f -name "vasprun.xml"','r')[1].readlines()]
        ...
        for x in fileList:#Loop over each config
            if args.type =='vasp':
                ...
                if os.path.exists(dirname+"/OUTCAR"):
                    try: noSCFiter=int(popen4("grep '\-\ Iteration' %s |wc -l"%(dirname+"/OUTCAR"))[1].readlines()[0][0:-1])
                    except: noSCFiter=0
                    try: nomaxSCF=int(grep("NELM",dirname+"/OUTCAR",-1).split("=")[1].split(";")[0])
                    except: nomaxSCF=0
                else:
                ...
                energies.append(float(popen4('grep  "%s" %s | tail -n 1'%(skey,x),'r')[1].readlines()[0].split()[-2])) 
...
#Convert absolute energies into hull_distance or relative energies here.
        ...
        data=popen4("casm query -k 'comp_n(Va)' hull_dist | grep SCEL1",'r')[1].readlines()#[-1][0:-1]
