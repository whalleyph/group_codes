...
TODO: tqdm for progress bar and parallelisation
"""
import pyfiglet
import argparse
...
def get_stems(inputs, use_parent=True):
    ...
    if len(stems) != len(set(stems)):
        if use_parent:
            new_stems = [
                f"{Path(input).parent}_{Path(input).stem}" for input in inputs]
        else:
            used = []
            new_stems = []
...
def process_atoms(atoms_list, fqhl, fl):
    ...
    if fqhl and not fl:
        if l < 4:
            ...
            q = int(l / 4) - 1
            h = int(l / 2) - 1
            processed = [atoms_list[0], atoms_list[q],
                         atoms_list[h], atoms_list[-1]]
            return processed
...
def convert_atoms(_file, stem, options, atom_index) -> int:
    try:
        ...
        if atom_index == -1:
            ...
            multiple = False
        elif options["fqhl"] == False:
            multiple = True
        atoms_list = process_atoms(
            raw_atoms_list, options["fqhl"], options["fl"])
...
symbols = []
options = {"E": args.energy, "F": args.forces,
           "V": args.virial, "H": args.hessian,
