...
parser.add_argument('-db','--db_list', nargs='*', type=str,required=0, default=['oqmd','aflow','mp','nmd','mpds','odbx'],help='Database list to get structures from, which support OPTIMADE framework. Check https://pymatgen.org/pymatgen.ext.optimade.html for up-to-date list')   #,'mcloud.li-ion-conductors'
...
for db in args.db_list:
 ...
 with OptimadeRester(aliases_or_resource_urls=db,timeout=60) as m:
  ...
  for stoich in args.stoich:
    ...
    if "-" in stoich: els=stoich.split('-');nels=len(els);form=None
    else            : form=stoich; els=None;nels=None

    print(stoich,form,els,nels)
    try:  data.update(m.get_structures(elements=els,nelements=nels,chemical_formula_hill=form))
    except:None
    #print(m.get_structures(elements=['Li','S'],nelements=2,chemical_formula_hill=None))

  try:
    key=list(data.keys())[0]; print(key)
    print('%d structure(s) were found for %s'%(len(data[key].values()),", ".join(args.stoich)))
  except:
  ...
  for dt in data[key]:
    ...
    sg=sg.replace('/','').replace('(','').replace(')','').replace(' ','')

    if db=='mp': outf=odir+"/"+form+'_'+sg+'_'+mid+'.res'
    else: outf=odir+"/"+form+'_'+sg+'_'+db+'-'+mid+'.res'
