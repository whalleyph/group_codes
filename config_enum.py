...
def find_equisites(atoms,all_W,all_w,tol=1e-6):
    ...
    for at,atom in enumerate(atoms): #coords of the current atom.
        ...
        for i,W in enumerate(all_W):#Find symmetry equi sites and match them to other available atoms.
                ...
                #assure the entries btw 0 and 1.
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ref', '--initCrysFile', type=str,help='File for obtaining the initial crystalline site (symmetry) information. Can be any type that ASE supports.')
    ...
    parser.add_argument('-excl', '--excludelist',nargs='*',type=str,help='List of crystalline sites to exclude from the configuration eneumeration (these atoms will not be considered even though selected by -n, -nl,-at or -wl earlier).  e.g. 1:4 24  for [1,2,3,4,24], atom numbering starts at 1.')
   ...
   #Get the  dopant/vacancy number list.
    ...
    if args.seq!=0 :
        ...
        #args.inpf = '../' + args.inpf
        args.initCrysFile = '../'+args.initCrysFile
        seq=args.seq
        ...
        for find,inpf in enumerate(args.inpf):
            ...
            if 'X' in args.aType and args.addint: #Find the potential interstices/vacancies, checkign the Wykoff site list for the space group. This must be done before finding the primitive and repetitions
                if args.initCrysFile: #if an input file for crsy. site info is provided.
                    ...
                    #if (len(args.inpf)>1 and find==0 ) or len(args.inpf==1_: 
            ...
            if not args.nosymm:
                ...
                #print (data)
                #print (len(data['rotations']))
                #print (data['site_symmetry_symbols'])
                print ("%d symmetry operations were detected. "%( len(data['translations'])))
                sg=get_spacegroup(cell, symprec=args.tol)
                print("Space group= %s with tol=%.2e"%(sg,args.tol))

                #all_W=np.unique(data['rotations'],axis=0) #does not work
                #all_w=np.unique(data['translations'],axis=0)
                ...
                if 0: #my own version also works well, and can be used.
                    ...
                    #print("Equi_site bk: %s"%(find_equisites(atoms,all_W,all_w,tol=args.tol)))
            ...
            #config_0 is the original structure.
            #ase.io.write(outdir+"config_0."+ext,atoms)
                ...
                for cind,conf in enumerate(c): #iterate over configurations. #combinations(atoms_list,r=i):
                    ...
                    if not args.nosymm:
                       #Now find the symetrically-equivalent configrations, by applying the symmops. Still iterating over the available configs. #Only include the selected ones for the given config/combination !!
                        ...
                        #for i,at in enumerate(atoms_list): #iterate over the atoms in the current combination
                       ...
                       #if cind>0:
                       #     flag=1
                       #     for eS in all_eSites: #check for coords is in all_eSites with subarrays for each dopant. (new style).
                        #    if cpos not in eS[i]: flag0

                        """
                        if cind>0: #This one is faster for higher no of dopants. Still gives more iredducible configs than the other, needs fixing.
                            crds=np.array([coords])
                            eSites=[]
                            for ei,eS in enumerate(all_eSites): eSites.extend(eS) #this faster than separate array (i.e. appending).
                            #eSites=np.array(eSites)#[0]
                            #print(eSites.shape,crds.shape)
                            #eSites=np.unique(eSites,axis=0)#[0]

                            #print(eSites.shape,crds.shape)
                            #print(eSites[:,:,:]-coords[0,:,:])

                            eSites=np.unique(eSites,axis=0)#[0]
                            dots=[np.dot((eSites[i,:,:]-crds[0,:,:]).T,(eSites[i,:,:]-crds[0,:,:])) for i in range(len(eSites))]

                                #for di,dt in enumerate(dots):
                            for dt in dots:
                                    #print(dt)
                                if np.isclose(np.sum(abs(dt)),0.): 
                                    #if not np.count_nonzero(dt): #np.all(dt==0.):
                                    #print ('matched',ei,cind,dt)
                                    eFlag=1;break
                                #elif np.isclose(np.sum(abs(dt)),float(len(conf)*len(conf))): 
                                #    print ('matched2',dt)

                            #print(dots.shape)
                            #print (np.where(dots,0.))
                                ...
                                for crd in equi: #loop over atoms in the equivalent configs This loop takes time.
                                    ...
                                    for k in range(len(conf)): #This is the bottleneck, should be fully numpied.
                                        for j in range(len(conf)): #compare with other atoms .
                                            ...
                                            #if np.allclose(coords,crd,atol=args.tol):  
                        ...
                        print(coords.T)
                        print(coords.T[:,0:3])
                        ncrds=np.zeros((len(coords[0]),len(coords)))
                        #for ni,nc in enumerate( ncrds):
                        #    ncrds[ni
                        print (ncrds)
                        etmp=[]
                        #etmp=SG.equivalent_sites(coords.T[:,0:3],onduplicates='replace')[0] 
                        for crd in coords.T:
                            print(crd)
                            etmp.append(SG.equivalent_sites(crd[0:3],onduplicates='replace')[0] )
                        print (etmp)
                        """
                        """
                        trans=np.zeros((len(all_W),4,4)) #no symm ops.
                        trans[:,0:3,0:3]=all_W[:,:,:]
                        trans[:,0:3,3]=all_w[:,0:3]
                        trans[:,3,:]=[0.,0.,0.,1.]
                        #new_coords=np.unique(np.dot(trans,coords),axis=0)
                        new_coords=np.dot(trans,coords)
                        ...
                        etmp=np.unique(new_coords,axis=0)
                        #print(etmp.shape,new_coords.shape)
                        """
                        ...
                        if 1: all_eSites.append(etmp) #use unique list here ??? original #Check if this is right !!
                            ...
                            for eS in all_eSites: #Seems like no identical entries in al_eSites.
                                #print(np.size(etmp), etmp.shape)
                                #print(np.size(eS),eS.shape)
                                #if np.array_equal(etmp,eS):
                                if np.allclose(etmp,eS):
                                #if not 0 in np.equal(etmp,eS):
                                #if (eS==etmp).all():
                        ...
                        """
                        if len(all_eSites)==0: all_eSites=deepcopy(etmp)
                        else:all_eSites=np.append(all_eSites,etmp,axis=0)
                        print(np.size(etmp), etmp.shape)
                        print(np.size(all_eSites),all_eSites.shape)

                        #all_eSites=np.append(all_eSites,etmp,axis=0)
                        print(np.shape(etmp))
                        print(etmp.shape)
                        #print(all_eSites)
                        print (etmp[8])
                        print(np.size(all_eSites))
                        all_eSites=np.unique(np.array(all_eSites),axis=0)
                        print(np.size(all_eSites))
                        #except:None
                        #print(all_eSites)
                        all_eSites=all_eSites.tolist()#[0]
                        #print(all_eSites)
                        """
        ...
        """%(cnt,occ,cnt)
                    ...
                    if mod_type in ['within','nearest']:
                            ...
                            for ind1 in ref_list:#indices
                                ...
                                for  at in search_list:
                                        #if len(sel_list)==mod_value: print ("burda");break # do not delete more than needed.
                                        ind2=at.index
                                        if ind1==ind2:continue
                                        #dist=atoms[ind1].get_distance(atoms[ind2],mic=True)
                                        #dist=atoms.get_distance(atoms[ind1],atoms[ind2],mic=True)
            ...
            if 1:
                ...
                for i,d in enumerate(degens): #Can be parralised to work on multiple configs simultaneously.
                    ...
                    if SG is None: 
                        try:SG=Popen4('symm -cl %s'%seed,timeout=30)[0][0]  #part of the airss package
                        except:SG=''
                ...
                #print ('Current',seqconfigs)
                #if len(args.inpf)>1:os.chdir('../')
