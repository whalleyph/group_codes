...
def stack(top_cell, bot_cell, vac=0):
...
def get_symmetry_translations(cell):
	...
	ts_sort = [x for x in ts_sort if x[2] < 1e-05]	

	tol = 1e-05
	t1 = ts_sort[0]	
	t1_cart = np.dot(cell.cell.T,t1)
	#print("t1, t1_cart", t1, t1_cart)
	for t in ts_sort:
		t_cart = np.dot(cell.cell.T,t)	
		#print("t, t_cart", t, t_cart)

		if np.linalg.norm(np.cross(t1_cart,t_cart)) > tol:
			#not parallel
			t2 = t
			t2_cart = t_cart
			break
	return t1_cart, t2_cart, np.linalg.norm(np.cross(t1_cart, t2_cart))	
...
if __name__== '__main__':
	...
	parser.add_argument("-n1","--max_atoms_1",default=40,type=int,help="max atoms in slab 1")	
	parser.add_argument("-n2","--max_atoms_2",default=40,type=int,help="max atoms in slab 1")	
	...
	for i in range(0,n):
		for j in range(0,m):
			...
			interface = stack(top_cell, bot_cell0, vac=args.vac)		
