...
def getPotcar(atom_order, PP_loc=os.environ["VASP_PP_PATH"]):
    def defs(sym):
        if sym in [
            "Li",
            "K",
    ...
    pots = " ".join([f"{PP_loc}/{defs[atom]}/POTCAR" for atom in atom_order])
    cmd = f"cat " + pots + " > POTCAR"
    os.system(cmd)


def processIncar(aimd_params):
    with open("INCAR", "r") as incar:
        incar_lines = incar.readlines()
    params = list(aimd_params.keys())
