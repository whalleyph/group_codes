...
if __name__ == '__main__':
    ...
    if args.type=="rdf" and args.skipSteps <100:
        print "Computational effort is not justified for a finer calculation of RDF. Setting the skip steps to 100..."
        args.skipSteps=100
    ...
    if args.aType: #Determine the atomic indices of the atoms with the desired type.
        if args.aType in ['all','a'] :atoms_list=range(noAtoms)
        else:
            atoms_list=[]
...
########################################
    ...
    if args.type=="rdf" or args.type=="a":
        ...
        nbins=None
        norm_by_V=1 # Normalize by volume of the spherical shell.
        dists={};        init_dists={}
        for i,at1 in enumerate(atoms_list): #can be parallelised over atoms.
            ...
            for at2 in range(len(aTypes)):
                ...
                for st in range(noSteps):
                    #x=minDist(coords_arr[st][at1],coords_arr[st][at2],lattice) #this matches with VMD using PBC (nly the peaks at longer dist 7-9 A are lower a.
                    #x=minDist_ASE(coords_arr[st][at1],coords_arr[st][at2],lattice) #Using the find_mic distance implemented in ASE.
                    x=dist(coords_arr[st][at1],coords_arr[st][at2]) #this matches with VMD not using PBC. #This should be used jointly  with unwrapped coordinates, otherwise no meaning for PBC systems.
                    #if x>rmax or x<rmin:continue #No need to store the too high and too low values. VMD matches when this is commented out !!!
